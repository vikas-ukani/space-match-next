                    ...
                    <div className="card-column">
                        ...
                        <div className="card-body p-0 pb-2">
                            ...
                            <div className="card-foot">
                                ...
                                {data.has_landlord_set_date == 0 && <p class="small font-italic text-warning">The Owner will contact you to arrange a site visit</p>}

                                {(data.site_visit_status == 1 && data.is_tenant_happy_with_space == 0) && (
                                    <>
                                        <p class="font-weight-medium text-center text-dark-grey mt-2">Confirmed site visit time :</p>
                                        <span class="btn cursor-unset">{data.site_visit_selected_date_time}</span>
                                    </>
                                )}
                                ...
                                {(data.has_landlord_set_date == 1 && data.site_visit_status == 1) && (
                                    <>
                                        {(data.is_tenant_happy_with_space == 0) && (
                                            <>
                                                <p class="small mt-2">Please select whether to book the Space or not : </p>
                                                <p class="font-weight-medium text-center text-dark-grey mt-2">Book Space ? </p>
                                            </>
                                        )}

                                        {(data?.tenant_detail?.is_fica_details_uploaded) && (
                                            <p class="small font-italic text-warning mt-2">
                                                In order to proceed with your booking, we will need your Fica details. Please click
                                                <a href="' + base_url + '/tenant-fica-details" target="_blank" class="font-weight-bold text-danger">HERE
                                ...
                                {(data?.has_landlord_set_date == 0 || data.site_visit_status == 1) && (
                                    <div className="d-flex justify-content-between">
                                        {data?.has_landlord_set_date == 0 && (
                                            <a class="btn btn-secondary withdraw-enquiry text-white">Withdraw</a>
                                        )}

                                        {(data.is_tenant_happy_with_space == 1)
                                            ? (
                                                <a class="btn btn btn-secondary view-enquiry text-white">View Space</a>
                                            )
                                            : (
                                                <a class="btn btn btn-secondary view-enquiry text-white">View</a>
                                            )
                                        }
                                    </div>
                                )}
                                ...
                                {(data.is_fica_verify_by_landlord == 2 && data.is_fica_sortout_by_admin == 0) && (
                                    <p class="small font-italic text-success mt-2">Landlord raised FICA issue, SpaceMatch might contact you.</p>
                                )}
