  ...
  if(sitevisit_enquiry.length > 0)
  {
    for(var sv=0;sv<sitevisit_enquiry.length;sv++)
    {
         ...
         var div_card = $('<div/>').addClass("card-column");
          ...
          div_created_date.append('<p>Enquiry date:'+sitevisit_enquiry[sv]['created_date']+'</p>');
          ...
          if(sitevisit_enquiry[sv]['has_landlord_set_date'] == 1)
          {
            var div_enquiry_by = $('<div/>').addClass("d-flex my-2 py-2");
            div_enquiry_by.append('<p class="small"><span class="text-grey mr-1">Enquiry by:</span> '+sitevisit_enquiry[sv]['tenant_detail']['firstname']+' '+sitevisit_enquiry[sv]['tenant_detail']['surname']+'</p>');
            card_body.append(div_enquiry_by);
          }
         ...
         // card_body.append(add_dates_blocks);
          ...
          {
              card_foot.append('<p class="small font-italic text-warning">Site visit times have been sent to '+sitevisit_enquiry[sv]['tenant_detail']['firstname']+' '+sitevisit_enquiry[sv]['tenant_detail']['surname']+'. You will receive email confirmation as soon as they select the site visit time.</p>');
              ...
              // }
              div_button.append('<a href="javascript:void(0)" data-id="'+sitevisit_enquiry[sv]['id']+'" class="btn btn btn-secondary view-enquiry">View Enquiry</a>');
          ...
          else
          {
              ...
              if(sitevisit_enquiry[sv]['site_visit_status'] == 1 && sitevisit_enquiry[sv]['is_tenant_happy_with_space'] == 0 && sitevisit_enquiry[sv]['is_fica_verify_by_landlord'] == 0)
              {
                  footer_class = ' mt-2';
                  card_foot.append('<p class="font-weight-medium text-center text-dark-grey mt-2">Confirmed site visit time :</p>');
                  card_foot.append('<span class="btn">'+sitevisit_enquiry[sv]['site_visit_selected_date_time']+'</span>');
              }

              if(sitevisit_enquiry[sv]['is_fica_verify_by_landlord'] == 1)
              {
                footer_class = ' mt-2';
                card_foot.append('<p class="font-weight-medium text-center text-dark-grey mt-2">Would you like to proceed and prepare the lease agreement for '+tenant_name+' ?</p>');

              }
              // if(sitevisit_enquiry[sv]['tenant_detail']['is_fica_details_uploaded'] == 0 && sitevisit_enquiry[sv]['site_visit_status'] == 1)
              // {
              //   var tenant_name = sitevisit_enquiry[sv]['tenant_detail']['firstname']+' '+sitevisit_enquiry[sv]['tenant_detail']['surname'];
              //   card_foot.append('<p class="small font-italic text-warning mt-2">Fica Details are not uploaded by '+tenant_name+' yet. Please wait for sometime or contact admin.</p>');
              // }

            // if(sitevisit_enquiry[sv]['tenant_detail']['is_verify_fica_category_by_admin'] == 1 && sitevisit_enquiry[sv]['is_tenant_happy_with_space'] == 0 && sitevisit_enquiry[sv]['site_visit_status'] == 1)
            if(sitevisit_enquiry[sv]['is_tenant_happy_with_space'] == 0 && sitevisit_enquiry[sv]['tenant_detail']['is_fica_details_uploaded'] == 1  && sitevisit_enquiry[sv]['site_visit_status'] == 1)
              {
                var tenant_name = sitevisit_enquiry[sv]['tenant_detail']['firstname']+' '+sitevisit_enquiry[sv]['tenant_detail']['surname'];
                card_foot.append('<p class="small font-italic text-warning mt-2">'+tenant_name+' has uploaded their Fica details. Waiting for move ahead.</p>');

              }
              if(sitevisit_enquiry[sv]['is_fica_verify_by_landlord'] == 2 && sitevisit_enquiry[sv]['is_tenant_happy_with_space'] ==1)
              {

                  if(sitevisit_enquiry[sv]['is_fica_sortout_by_admin'] == 0)
                  {
                    var div_button1 = $('<div/>').addClass("d-flex justify-content-between"+footer_class);
                    div_button1.append('<span class="btn btn-danger cursor-unset">Decline FICA</span>');
                    card_foot.append('<p class="small font-italic text-warning mt-2">Your Spacematch representative will be in touch shortly to discuss the FICA verification.</p>');
                    card_foot.append(div_button1);
                    card_foot.append('<p class="small font-italic text-success mt-2">SpaceMatch are working on FICA issue raised by you.</p>');
                  }
                  if(sitevisit_enquiry[sv]['is_fica_sortout_by_admin'] == 1)
                  {
                      var div_button1 = $('<div/>').addClass("d-flex justify-content-between"+footer_class);
                      div_button1.append('<a href="javascript:void(0)" data-id="'+sitevisit_enquiry[sv]['id']+'" data-user-id="'+sitevisit_enquiry[sv]['added_by']+'" class="btn btn-primary accept-view-fica-details">View & Accept FICA Details</a>');
                      card_foot.append(div_button1);
                  }
              }
              ...
              if( ((is_staff == '1' && user_permission_name == 'write') || is_staff == '0'))
              {
                  ...
                  //if(sitevisit_enquiry[sv]['has_landlord_set_date'] == 1 && sitevisit_enquiry[sv]['site_visit_status'] == 1  && sitevisit_enquiry[sv]['is_tenant_happy_with_space'] == 1) // has_landlord_set_date = 0 && site_visit_status = 0
                  if(sitevisit_enquiry[sv]['is_fica_verify_by_landlord'] == 1)
                  {
                      var div_button = $('<div/>').addClass("d-flex justify-content-between"+footer_class);
                      div_button.append('<a href="javascript:void(0)" data-id="'+sitevisit_enquiry[sv]['id']+'" class="btn btn-danger decline-enquiry">Decline Enquiry</a>');
                      ...
                      div_button1.append('<a href="javascript:void(0)" data-id="'+sitevisit_enquiry[sv]['id']+'" class="btn btn-primary prepare-agreement">Prepare Agreement</a>');
                      card_foot.append(div_button1);
                  }
                  if(sitevisit_enquiry[sv]['is_fica_verify_by_landlord'] == 0 && sitevisit_enquiry[sv]['is_tenant_happy_with_space'] ==1)
                  {
                      var div_button1 = $('<div/>').addClass("d-flex justify-content-between"+footer_class);
                      div_button1.append('<a href="javascript:void(0)" data-id="'+sitevisit_enquiry[sv]['id']+'" data-user-id="'+sitevisit_enquiry[sv]['added_by']+'" class="btn btn-primary accept-view-fica-details">View & Accept FICA Details</a>');
                      card_foot.append(div_button1);
              ...
              if(sitevisit_enquiry[sv]['is_tenant_happy_with_space'] == 1 && sitevisit_enquiry[sv]['tenant_detail']['is_verify_fica_category_by_admin'] == 0)
              {
                footer_class = ' mt-2';
                card_foot.append('<p class="small font-italic text-warning mt-2">Please note : FICA details are NOT verified by SpaceMatch.</p>');
              }
    ...
    $('.prepare-agreement').click(function () {
      var enquiryId = $(this).data('id');
      prepareAgreement(enquiryId,0);
    });
    $('.accept-view-fica-details').click(function () {
      var enquiryId = $(this).data('id');
      var userId = $(this).data('user-id');
      viewAcceptFicaDetails(enquiryId, userId);
    });
    ...
    for(var pa=0;pa<pending_enquiry.length;pa++)
    {
          ...
          if( (is_staff == '1' && user_permission_name == 'write') || is_staff == '0')
          {
              if(pending_enquiry[pa]['site_visit'] == 0)
              {
                ...
                div_button1.append('<a href="javascript:void(0)" data-id="'+pending_enquiry[pa]['id']+'" class="btn btn-primary accept-and-send-agreement">accept & prepare agreement</a>');
                card_foot.append(div_button1);
              }
          }
          div_card.append(card_foot);
...
function sendAgreementToTenant(enquiryId)
{
...
function landlordLegalAgreement(response)
{
  ...
  if(agreement_enquiry.length > 0)
  {
    for(var ae=0;ae<agreement_enquiry.length;ae++)
    {
          ...
          var div_day_price = $('<div/>').addClass("d-flex justify-content-between border-bottom pb-3");
          div_day_price.append('<p class="btn cursor-unset">Lease period :<br/>'+agreement_enquiry[ae]['property_rate_details']['total_days']+' Days</p>');
          ...
          var div_tenant_detail = $('<div/>').addClass("border-top pt-2 pb-1");
          div_tenant_detail.append('<p class="font-weight-medium">Space User Details:</p>');
          ...
          div_tenant_name_mobile.append('<p class="small text-grey">+27 '+agreement_enquiry[ae]['tenant_detail']['mobile']+'</p>');
          div_tenant_detail.append(div_tenant_name_mobile);
         ...
         // if(agreement_enquiry[ae]['is_agreement_send_to_landlord'] == 1 && agreement_enquiry[ae]['is_sign_by_landlord'] == 0)
          if(agreement_enquiry[ae]['is_prepare_agreement_by_landlord'] == 1 && agreement_enquiry[ae]['is_agreement_send_to_tenant'] == 0)
          {
               footer_class  = ' mt-2';
               var legal_agreement_div = $('<div/>').addClass("d-flex justify-content-between align-items-center");

               legal_agreement_div.append('<p class="small mt-2">Would you like to send the agreement to '+agreement_enquiry[ae]['tenant_detail']['firstname']+' '+agreement_enquiry[ae]['tenant_detail']['surname']+' ? </p>');
...
//legal_agreement_div.append('<p class="text-primary small font-weight-medium">Legal Agreement is ready and sent to your register email.Please signed it.</p>');
          ...
          //if(agreement_enquiry[ae]['is_agreement_send_to_landlord'] == 1 && agreement_enquiry[ae]['is_sign_by_landlord'] == 0)
          // if(agreement_enquiry[ae]['is_agreement_send_to_landlord'] == 1 && agreement_enquiry[ae]['is_sign_by_tenant'] == 0)
          // {
          //      footer_class  = ' mt-2';
          //      var legal_agreement_div = $('<div/>').addClass("d-flex justify-content-between align-items-center");
          ...
          if(agreement_enquiry[ae]['is_agreement_send_to_tenant'] == 1 && agreement_enquiry[ae]['is_sign_by_tenant'] == 0)
          {
              footer_class  = ' mt-2';
              var legal_agreement_div = $('<div/>').addClass("d-flex justify-content-between align-items-center");
              legal_agreement_div.append('<p class="text-warning small font-weight-medium">Waiting for '+agreement_enquiry[ae]['tenant_detail']['firstname']+' '+agreement_enquiry[ae]['tenant_detail']['surname']+' to sign the agreement.</p>');
              card_foot.append(legal_agreement_div);
          }

          if(agreement_enquiry[ae]['is_sign_by_landlord'] == 0 && agreement_enquiry[ae]['is_sign_by_tenant'] == 1)
          {
              footer_class  = ' mt-2';
              var legal_agreement_div = $('<div/>').addClass("d-flex justify-content-between align-items-center");
              legal_agreement_div.append('<p class="text-warning small font-weight-medium">'+agreement_enquiry[ae]['tenant_detail']['firstname']+' '+agreement_enquiry[ae]['tenant_detail']['surname']+' has signed agreement. Please review it and sign it.</p>');
              card_foot.append(legal_agreement_div);
          }
          var div_button = $('<div/>').addClass("d-flex justify-content-between"+footer_class);

          //if(agreement_enquiry[ae]['is_prepare_agreement_by_landlord'] == 1 && agreement_enquiry[ae]['is_agreement_send_to_landlord'] == 0)
          if(agreement_enquiry[ae]['is_prepare_agreement_by_landlord'] == 1 && agreement_enquiry[ae]['is_agreement_send_to_tenant'] == 0)
          {

            if( (is_staff == '1' && user_permission_name == 'write') || is_staff == '0')
            {
              div_button.append('<a href="javascript:void(0)" data-id="'+agreement_enquiry[ae]['id']+'" class="btn btn-primary send-agreement-only">Yes</a>');
              div_button.append('<a href="javascript:void(0)" data-id="'+agreement_enquiry[ae]['id']+'" class="btn btn-danger decline-enquiry">No</a>');
            }
          }
          else
          {
    ...
    $('.send-agreement-only').click(function() {
      var enquiryId = $(this).data('id');
      sendAgreementToTenant(enquiryId);
    });
...
function landlordPayment(response)
{
  ...
  if(payment_enquiry.length > 0)
  {
    for(var pe=0;pe<payment_enquiry.length;pe++)
    {
          ...
          var tenant_name = payment_enquiry[pe]['tenant_detail']['firstname']+' '+payment_enquiry[pe]['tenant_detail']['surname'];
          //Card Cloumn Start
          var card_class = '';
          ...
          div_day_price.append('<p class="btn cursor-unset '+day_class+'">Lease period :<br/>'+payment_enquiry[pe]['property_rate_details']['total_days']+' Days</p>');
          ...
          div_note.append('<p><span class="small font-weight-medium">Please note: The amount shown above does not reflect a deposit or any other amounts specific to your agreement that have been agreed with '+tenant_name+'</span></p>');
          ...
          var div_button = $('<div/>').addClass("d-flex justify-content-between mt-2");
          div_button.append('<a href="javascript:void(0)" data-id="'+payment_enquiry[pe]['id']+'" class="btn btn-secondary view-enquiry">View enquiry</a>');
          if( (is_staff == '1' && user_permission_name == 'write') || is_staff == '0')
          {
...
function prepareAgreement(enquiryId,is_accept)
{

    $.ajax({
      url: api_url+"prepare-agreement-for-tenant",
      method: 'POST',
      data:{enquiry_id : enquiryId, is_accept: is_accept},
      headers: {
        'Authorization': 'Bearer '+ token,
        "Accept": "application/json",
      },
      ...
      success: function (data) {
          $(".loader").hide();
          if(data.success == true){
            $.redirect(base_url + '/landlord-agreement-template','','GET');
          }
      },
      error: function (data) {
        $(".loader").hide();
...
$("#decline_fica").click(function(){
  var enquiryId = $('#modalViewAcceptFICA #enquiryId').val();
  $.ajax({
            url: api_url+"landlord-decline-fica",
            method: 'post',
            data: {enquiry_id: enquiryId},
            headers: {
              'Authorization': 'Bearer '+ token,
            ...
            success: function (data) {
                $(".loader").hide();
                if(data.success == true){
                  $('#modalViewAcceptFICA').modal('hide');
                  setTimeout(function(){
                    pipleline();
                  }, 1000);
                }
...
$("#accept_fica").click(function(){
  var enquiryId = $('#modalViewAcceptFICA #enquiryId').val();
  $.ajax({
          url: api_url+"landlord-accept-fica",
          method: 'post',
          data: {enquiry_id: enquiryId},
          headers: {
            'Authorization': 'Bearer '+ token,
